#!/bin/bash
#===========================================
# script: run_postproc_local.bash
# intent: to be called from the command line, interactively
# syntax: ./run_postproc_local.bash -m {mech} -k {case} f {flagfile}
...
#-------------------
# If using Cheyenne, load correct modules

local=$(hostname)
cisl='cheyenne'

  if [[ "$local" == *"$cisl"* ]] ;then
    echo " I see you're using cheyenne: reloading modules"
    module unload intel
...
# ... IF READ IN FROM file "setup.dat"
if [ ! -e "setup.dat" ] ; then
...
if [ -e "${postproc_source}/${flags_input}" ] ; then
  ...
  ln -sf ${postproc_source}/${flags_input} ${postproc_workingdir}/postproc_flags.input
...
echo linking pvap data from ${gecko_workingdir}/${mech_name}.sat 
...
if [ ! -e $gecko_source/OBJ/geckolib.a ] ; then
  ...
  make cm
  #exit
else # check date on existing geckolib.a
