#!/bin/bash
#===========================================
# PURPOSE: run boxmodel spinup on cheyenne
# CALLING SYNTAX: ./run_boxspin_cheyenne.bash -m (mech) -k (keyfile) -v (previous run) -p (photfile)
...
runlength=65000  # each run will be 86400 seconds
...
#--
while getopts "a:b:c:d:f:h:i:k:l:m:n:p:r:s:t:v:w:x" opt
do
  case "$opt" in
# general files
    a ) pathfile=$OPTARG ;;
...
# see compile_boxmod.bash script
  if [ ${counter} -eq 1 ]; then
    echo "submitting job to compile BOXMOD on cheyenne"
      if [ -n "${depend}" ] ; then
        write_cheyenne_monoproc_dependentscript\
          box_${run_name}_compilation.bash\
          box_${run_name}_compilation\
          ${walltime_compilation}\
          out_box_${run_name}_compilation\
          err_box_${run_name}_compilation\
          ${depend}\
    ...
    compilation_job_id=`qsub -h ${home_dir}/GENERATED_SCRIPTS/box_${run_name}_compilation.bash`
  fi
  ...
  echo linking gitinfo file ${gecko_mechdir}/${mech}.gitinfo
  ...
  # if [ ! -e ${boxmod_wkdir}/BOXMOD ] ; then
  #    echo "could not find boxmodel executable for sub_run #${counter}"
  #    exit
  # fi
  ...
  echo linking diffusion volumn file ${gecko_mechdir}/${mechname}.difv ...
...
# first run
  ...
  #if ${prevout} provided, set PREV = 1
      if grep -q "PREV" ${boxmod_wkdir}/indat.key ; then
        sed -i 's/PREV 0/PREV 1/g' ${boxmod_wkdir}/indat.key
      else
        sed -i '1 i\PREV 1' ${boxmod_wkdir}/indat.key
...
# replace any existing "PREV 1" line (for documentation purposes)
  ...
  if [[ -n "${prevout}" || ${counter} -gt 1 ]]; then
    ...
    #-- check for previous output files outdat.ppa, outdat.ppf
    if [ -n "${prevout}" ]; then
      if [ ! -e ${boxmod_prevdir}/outdat.ppf ]; then
        echo previous output file \'${boxmod_prevdir}/outdat.ppf\' doesn\'t exist
        exit
      fi
      if [ ! -e ${boxmod_prevdir}/outdat.ppa ]; then
        echo previous output file \'${boxmod_prevdir}/outdat.ppa\' doesn\'t exist
        exit
      fi
    fi
...
#==============================================================#
## section for auto-adding precursor concs to keyfile (OPTIONAL)
#==============================================================#
  ...
  if [ -n "${library_flag}" ]; then
    ...
    if [ ! -e ${boxmod_outdir}/spinup_spinup_${keyfile}/steadystate.key ]; then
    ...
    echo adding steady-state spinup ${boxmod_outdir}/spinup_spinup_${keyfile}_spinup_${keyfile}_1_1/steadystate.key...
    echo to ... ${boxmod_wkdir}/indat.key
      cp ${boxmod_outdir}/spinup_spinup_${keyfile}_spinup_${keyfile}_1_1/steadystate.key ${boxmod_wkdir}/init_steadystate.key
...
# no need for himem scripts, boxmod runs rarely take more that 2000MB memory (tested for 800000 species)
  if [[ ${counter} -eq 1 ]]; then
    write_cheyenne_dependentscript\
        box_${run_name}_${counter}_${numruns}.bash\
        box_${run_name}_${counter}_${numruns}\
        ${walltime_box}\
        ${nthreads}\
        out_box_${run_name}_${counter}_${numruns}\
        err_box_${run_name}_${counter}_${numruns}\
  ...
  current_wkdir=`pwd`
  cd ${boxmod_wkdir} 
  previous_job_id=`qsub ${home_dir}/GENERATED_SCRIPTS/box_${run_name}_${counter}_${numruns}.bash`
  cd ${current_wkdir}

  write_cheyenne_monoproc_dependentscript\
     ppr_${run_name}_${counter}_${numruns}.bash\
     ppr_${run_name}_${counter}_${numruns}\
     ${walltime_post}\
     out_ppr_${run_name}_${counter}_${numruns}\
     err_ppr_${run_name}_${counter}_${numruns}\
     afterany:${previous_job_id} \
     ${home_dir}/SCRIPTS/run_postproc_cheyenne.bash\
     ${mechname}\
     ${keyfile}_${confile}_${counter}_${numruns}

  echo ../submitting postprocessing script
  qsub  ${home_dir}/GENERATED_SCRIPTS/postproc_${run_name}_${counter}_${numruns}.bash
done
