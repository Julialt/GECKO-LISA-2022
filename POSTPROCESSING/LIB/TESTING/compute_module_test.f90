...
!-----------------------------------------------------------------------
    ...
    allocate(environ_params(rdat, ind_pres))
...
!-----------------------------------------------------------------------
    ...
    if(nprecu .eq. 1) then
      ...
      allocate(precursor_lifetime(rdat, 2))
...
!-----------------------------------------------------------------------
    ...
    allocate(carb_dist(rdat, ncols), mass_dist(rdat, ncols))
    header(1) = "Time [s]"
    do iphase = 1, nphase
      select case (iphase)
        case (GAS)
...
!-----------------------------------------------------------------------
    ...
    allocate(mass_precu(rdat), mass_soa(rdat), soa_yield(rdat, 2))
...
!-----------------------------------------------------------------------
    ...
    func_descs(21) =  ">C.(OO.)"
    func_descs(22) =  "-OSO3"
    allocate(rof_c_aer(rdat, function_types+1), &
             rof_c_gas(rdat, function_types+1), &
             rof_cp_gas(rdat, function_types+1), &
             rof_cp_aer(rdat, function_types+1), &
             massfunc_aer(rdat, function_types+1), &
             massfunc_gas(rdat, function_types+1), &
             ncarbon_aer(rdat), &
             ncarbon_gas(rdat), &
             ncarbon_tot_gas(rdat), &
             ncarbon_tot_aer(rdat))
    ncarbon_gas = 0.
    ncarbon_tot_gas = 0.
    ncarbon_tot_aer = 0.
    ncarbon_aer = 0.
...
!-----------------------------------------------------------------------
    ...
    allocate(carbon_chain_dist_gas(rdat, maxc + 1), &
             carbon_chain_dist_aer(rdat, maxc + 1), &
             header(maxc + 1))
...
!-----------------------------------------------------------------------
    ...
    topN = 10
    allocate(top_chon_concentrations(rdat, topN+2 ), &
             header(topN + 2), &
...
!-----------------------------------------------------------------------
    ...
    allocate(atom_ratios(rdat, 4), header(4), sumnc(rdat))
...
!-----------------------------------------------------------------------
    ...
    allocate(ams_factors(rdat, nfactors + 1), &
             header(nfactors + 1))
...
! NB: formula truncates top bin if (maxpvap-minpvap)/resolution is non-integral
    ...
    allocate(upperlim(nbins), lowerlim(nbins), &
             pvap_distribution_ppbC(rdat*nbins, ncols), &
             pvap_distribution_ug(rdat*nbins, ncols), &
...
! NB: formula truncates top bin if (maxkh-minkh)/resolution is non-integral
    ...
    allocate(upperlim(nbins), lowerlim(nbins), &
             kh_distribution_ppbC(rdat*nbins, ncols), &
             kh_distribution_ug(rdat*nbins, ncols), &
             header_ppbC(ncols), header_ug(ncols))
...
!-------------------------------------------------------------------
    ...
    allocate(top_species_names(N), top_species_indx(N), &
             top_species_molec(rdat, N +2), &
             top_species_ppb(rdat, N +2), &
...
!-------------------------------------------------------------------
    ...
    N = n_topspecies
    allocate(top_species_names(N), top_species_indx(N), &
             top_species_concentrations(rdat, N +2), &
...
!-------------------------------------------------------------------
    ...
    allocate(contributing_species(rdat, 3), header(3), temp_conc(numsp))
    header(1) = "Time [s]"
    header(2) = "50% species"
    header(3) = "90% species"
    do iphase = GAS, AEROSOL, AEROSOL-GAS
...
!-------------------------------------------------------------------
    ...
    allocate(header(nsel+1), selected_species_conc(rdat, nsel +1 ),selected_species_ppb(rdat, nsel +1 ),sel_indices(nsel))
...
! find target species index in alphabetically sorted species list
    ...
    do itime = 1,rdat
      !print*,"itime = ",itime
      !CALL eznc_get_3Dreal(ncid,"reacrate", maxre,  mbox,  rdat, &
...
!-------------------------------------------------------------------
    ...
    allocate(elements_contrib(rdat, 5), header(5))
...
! according to Riemer & West (2013)
    ...
    allocate(h(rdat, 5), header(5), cumulative_total_mass(rdat))
    header(ind_time) = "Time [s]"
...
!-------------------------------------------------------------------
    ...
    allocate(chochonfreq_molec(rdat*nbins, ncols), &
...
!---------------------------------------------------------------------
    ...
    allocate(mass_spec(rdat*mass_bins%nbins, ncols), header(ncols))
...
!-----------------------------------------------------------------------
    ...
    allocate(bubble(rdat*pvap_bins%nbins*osc_bins%nbins, ncols), &
             header(ncols))
    do iphase = 1, nphase
      select case (iphase)
        case (GAS)
...
!-----------------------------------------------------------------------
    ...
    allocate( &
      functionalization(rdat, IND_FTOTAL), &
      substitution(rdat, IND_STOTAL), &
...
!    enddo
!
!    allocate(osc_nc(rdat*nphase*nosc_bin*nnc_bin, 5))
!    osc_nc = 0.
!
!    do itime = 1, rdat
!      do ispec = 1, numsp
...
!-----------------------------------------------------------------------
  ...
  ! two more bins added: smaller than minpvap, and larger than maxpvap
    ...
    allocate(upperlim(nbins), lowerlim(nbins), &
             pvap_distribution_ppbC(rdat*nbins, LAST_PHASE_shrink+2+n_VBS_param), &
             pvap_distribution_ug(rdat*nbins, LAST_PHASE_shrink+2+n_VBS_param), &
             header_ppbC(LAST_PHASE_shrink+2+n_VBS_param), header_ug(LAST_PHASE_shrink+2+n_VBS_param))    
  allocate(atom_ratios(rdat, 4))
    allocate(mass_precu(rdat), conc_precu(rdat), mass_soa(rdat, nbins), mass_cg(rdat, nbins))
  allocate(soa_yield(rdat, nbins), cg_yield(rdat, nbins))
...
!--------------------------------------------------------------------------
      ...
      allocate(atom_ratios_frag(rdat, 4), atom_ratios_dimer(rdat,4), sumnc(rdat))
