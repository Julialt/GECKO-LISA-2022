...
! INTERNAL VARIABLES
      ...
      SELECT CASE (status)
        ...
        CASE (42)
          ...
          report = "String match to errnam in use"
        CASE (43)
          errnam = "NC_ENOTATT      "
...
! write variable
      ...
      status = NF90_PUT_VAR(ncid,varid, &
                            ncvals(start1:end1,start2:start2), &
                            start=(/start1,start2/), &
                            count=(/count1,count2/))
...
! INTERNAL VARIABLES
      ...
      INTEGER varid
      INTEGER count1,count2,count3
      REAL,DIMENSION(end1-start1+1,start2,start3) :: ncvals
      CHARACTER(25) text
...
! convert 1-D array into 3-D array slab of dimensions (count1,1,1)
      ncvals(start1:end1,start2,start3) = invals
...
! write variable
      text   ="NF90_PUT_VAR "//VarName
      status = NF90_PUT_VAR(ncid,varid,  & 
                        ncvals(start1:end1,start2:start2,start3:start3),&
                        start=(/start1,start2,start3/),&
                        count=(/count1,count2,count3/))
...
! -------------------------------
!      SUBROUTINE eznc_get_1Dint(ncid,VarName,dimval,outvals, &
      SUBROUTINE eznc_get_1Dint(ncid,VarName,outvals, &
                           start1,end1,stride1)
...
! INPUT VARIABLES
      ...
      INTEGER ncid,start1,end1,stride1
      CHARACTER(*) VarName
! OUTPUT VARIABLES
      INTEGER,DIMENSION(INT((end1-start1)/stride1)+1) :: outvals
! INTERNAL VARIABLES
      INTEGER status
      INTEGER varid
      INTEGER count1
      ...
      outvals(:) = 0
      count1 = INT((end1-start1)/stride1)+1
...
! read variable
      text   ="NF90_GET_VAR "//VarName
      !status = NF90_GET_VAR(ncid,varid,outvals(start1:end1), &
      status = NF90_GET_VAR(ncid,varid,outvals, &
                            start=(/start1/), &
                            count=(/count1/), &
                            stride=(/stride1/))
      IF (status/=NF90_NOERR) CALL eznc_handle_err(status,text)
...
! -------------------------------
!      SUBROUTINE eznc_get_1Dreal(ncid,VarName,dimval,outvals, &
      SUBROUTINE eznc_get_1Dreal(ncid,VarName,outvals, &
                                 start1,end1,stride1)
...
! INPUT VARIABLES
      ...
      CHARACTER(*) VarName
! OUTPUT VARIABLES
      REAL,DIMENSION(INT((end1-start1)/stride1)+1) :: outvals
! INTERNAL VARIABLES
      INTEGER status
      INTEGER varid
      INTEGER count1
...
!      count1=end1-start1+1
      ...
      print*,"eznc_get_1Dreal: ",VarName,count1
...
!      SUBROUTINE eznc_get_1Dchar(ncid,VarName,nchar,dimval, &
!                                      outvals,start1,end1)
      SUBROUTINE eznc_get_1Dchar(ncid,VarName,nchar, &
                                      outvals,start1,end1,stride1)
...
! INPUT VARIABLES
      ...
      INTEGER ncid,nchar,start1,end1,stride1
      CHARACTER(*) VarName
! OUTPUT VARIABLES
      CHARACTER(nchar) outvals(INT((end1-start1)/stride1)+1)
! INTERNAL VARIABLES
      INTEGER status
      INTEGER varid
      INTEGER count1
...
! -------------------------------
!      SUBROUTINE eznc_get_2Dint(ncid,VarName,dim1,dim2,outvals, &
      SUBROUTINE eznc_get_2Dint(ncid,VarName,outvals, &
                           start1,end1,stride1,start2,end2,stride2)
...
! INTERNAL VARIABLES
      ...
      INTEGER varid
      INTEGER count1,count2
      !INTEGER,DIMENSION(dim,dim2) :: values
      CHARACTER(40) text
      ...
      outvals(:,:) = 0
      count1=INT((end1-start1)/stride1)+1
      count2=INT((end2-start2)/stride2)+1
...
! -------------------------------
!      SUBROUTINE eznc_get_2Dreal(ncid,VarName,dim1,dim2,outvals, &
      SUBROUTINE eznc_get_2Dreal(ncid,VarName,outvals, &
                           start1,end1,stride1,start2,end2,stride2)
...
! INPUT VARIABLES
      ...
      CHARACTER(*) VarName
! OUTPUT VARIABLES
      REAL,DIMENSION(INT((end1-start1)/stride1)+1,&
                     INT((end2-start2)/stride2)+1) :: outvals
! INTERNAL VARIABLES
      INTEGER status
      INTEGER varid
      INTEGER count1,count2
...
!                outvals,start1, end1, start2, end2, start3, end3)
      SUBROUTINE eznc_get_3Dreal(ncid, VarName, outvals, &
                                 start1, end1, stride1, &
                                 start2, end2, stride2, &
                                 start3, end3, stride3)
...
! INPUT VARIABLES
      ...
      INTEGER stride1,stride2, stride3
      CHARACTER(*) VarName

! OUTPUT VARIABLES
      REAL,DIMENSION(INT((end1-start1)/stride1)+1, &
                     INT((end2-start2)/stride2)+1, &
                     INT((end3-start3)/stride3)+1) :: outvals
! INTERNAL VARIABLES
      INTEGER status
      INTEGER varid
      INTEGER count1, count2, count3
      ...
      count1=INT((end1-start1)/stride1)+1
      count2=INT((end2-start2)/stride2)+1
      count3=INT((end3-start3)/stride3)+1
      print*,"eznc_get_3Dreal:",VarName, count1,count2,count3
      !print*,start1,start2,start3
      !print*,count1,count2,count3
      !print*,dim1,dim2,dim3

      print*,lbound(outvals, 1),lbound(outvals, 2),lbound(outvals, 3)
      print*,ubound(outvals, 1),ubound(outvals, 2),ubound(outvals, 3)
...
! read variable
      ...
      status = NF90_GET_VAR(ncid,varid,outvals, &
                            start=(/start1,start2,start3/),&
                            count=(/count1,count2,count3/),&
                            stride=(/stride1,stride2,stride3/))
      IF (status/=NF90_NOERR) call eznc_handle_err(status,text)
